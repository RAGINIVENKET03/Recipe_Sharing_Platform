...
const addRecipe=async(req,res)=>
{
    ...
    catch(error)
    {
        ...
        return res.status(400).send({message:"Error in adding the recipe"})
    }
}
const updateRecipeById=async(req,res)=>
{
    ...
    try{
        ...
        if(!updateRecipe){
            return res.status(404).send({message:"Error in updating the Recipe"});
        }
        return res.status(200).send({updateRecipe:updateRecipe});
    }
    catch(error){
        console.error("Error in updating the recipe:",error.message);
        return res.status(400).send({message:"Error in updating the recipe"});
    }
}
const deleteRecipeById=async(req,res)=>
{
    ...
    try{
        ...
        return res.status(400).send({message:"Error in deleting the recipe"});
    }
}
